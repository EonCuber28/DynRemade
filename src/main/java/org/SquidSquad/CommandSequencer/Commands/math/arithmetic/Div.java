...
public class Div extends Command {
    ...
    public void run(){
        ...
        if (in1 != null || in2 != null){
            if (in1 != null && in2 != null){
                varManager.getVar(OutVarID).Div(
                        new DynNumber(in1),
                        new DynNumber(in2));
            } else if (in1 != null){
                switch (inCon) {
                    case I1O1 -> varManager.getVar(OutVarID).Div(
                            new DynNumber(in1),
                            varManager.getVar(InVarIDs[1]));
                    case I2 -> varManager.getVar(OutVarID).Div(
                            new DynNumber(in1));
                }
            } else {
                switch (inCon) {
            ...
            switch (inCon) {
                case I2O1 ->
                        varManager.getVar(getOutVarID()).Div(varManager.getVar(InVarIDs[0]), varManager.getVar(InVarIDs[1]));
