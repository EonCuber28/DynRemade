...
public class For extends Command {
    ...
    @Override
    public void run(){
        Variable loopedVar = super.varManager.getVar(super.getInVarIDs()[0]);
        switch (loopedVar.getType()){
            case List -> {
                ArrayList<Variable> arrayItems = (ArrayList<Variable>)loopedVar.getValue();
                for (Variable item : arrayItems){
                    super.varManager.getVar(super.getOutVarID()).setVariable(item);
            ...
            case Number -> {
                ...
                for (int number : NumberRange){
                    super.varManager.getVar(super.getOutVarID()).setValue(number);
                    for (Command cmd : innerCommands){
                        cmd.run();
                    }
                }
            ...
            case String -> {
                for (char chunk : ((String)loopedVar.getValue()).toCharArray()){
                    super.varManager.getVar(super.getOutVarID()).setValue(chunk);
                    for (Command cmd : innerCommands){
                        cmd.run();
                    }
                }
